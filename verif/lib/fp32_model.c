...
uint32_t c_fp32_add(uint32_t a, uint32_t b) {
    float fa = u32_to_float(a);
    float fb = u32_to_float(b);
    float fresult = fa + fb;
    return float_to_u32(fresult);
}

// Multiply two fp32 numbers
uint32_t c_fp32_mul(uint32_t a, uint32_t b) {
    float fa = u32_to_float(a);
    float fb = u32_to_float(b);
    return float_to_u32(fa * fb);
}

// Divide two fp32 numbers
uint32_t c_fp32_div(uint32_t a, uint32_t b) {
    float fa = u32_to_float(a);
    float fb = u32_to_float(b);
    return float_to_u32(fa / fb);
}
...
// Fused multiply-add
uint32_t c_fp32_mul_add(uint32_t a, uint32_t b, uint32_t c) {
    float fa = u32_to_float(a);
    float fb = u32_to_float(b);
    float fc = u32_to_float(c);
    return float_to_u32(fmaf(fa, fb, fc));
}

// Square root
uint32_t c_fp32_sqrt(uint32_t a) {
    float fa = u32_to_float(a);
    return float_to_u32(sqrtf(fa));
}
